#!/usr/bin/env python3
"""Score a test submission against the frozen class-reviewed ground truth.

This is intentionally isolated from the training pipeline. It reads a
submission CSV and the exported class-reviewed/manual test annotations, then writes
metrics under working/reports/gt_scoring/results/.
"""
...
DEFAULT_GT = PROJECT_ROOT / "working/gt_scoring/ground_truth/test_ground_truth.csv"
DEFAULT_CLASS_MAP = PROJECT_ROOT / "working/gt_scoring/ground_truth/pill_class_number_map_74.csv"
DEFAULT_MODIFIED_MANIFEST = (
    PROJECT_ROOT
    / "working/gt_scoring/ground_truth/manual_review_reconciliation.csv"
)
DEFAULT_UNKNOWN_IGNORE = PROJECT_ROOT / "working/test_annotations/test_unknown_ignore_boxes.csv"
DEFAULT_OUT_ROOT = PROJECT_ROOT / "working/gt_scoring/results"
...
def parse_args() -> EvalInputs:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate a test submission against the frozen ground truth. "
    ...
    parser.add_argument(
        ...
        default="all",
        help=(
            "all: every ground truth image; modified: images touched by correction log; "
...
def load_inputs(inputs: EvalInputs) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict[int, str]]:
    ...
    if not inputs.ground_truth.exists():
        raise FileNotFoundError(inputs.ground_truth)

    n_to_cat, cat_to_name = load_class_map(inputs.class_map)
    pred = pd.read_csv(inputs.submission)
    gt = pd.read_csv(inputs.ground_truth)

    pred = pred.copy()
    gt = gt.copy()
    pred["category_id"] = resolve_category_id(pred, n_to_cat, "submission")
    gt["category_id"] = resolve_category_id(gt, n_to_cat, "ground truth")

    pred = normalize_boxes(pred, "submission")
    gt = normalize_boxes(gt, "ground truth")
...
def plot_confusion(confusion: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.set_title("Ground Truth confusion matrix")
    fig.tight_layout()
    fig.savefig(out_path)
    plt.close(fig)


def coco_eval(pred: pd.DataFrame, gt: pd.DataFrame, cat_to_name: dict[int, str]) -> float:
    ...
    coco_gt = COCO()
    coco_gt.dataset = {
        "info": {"description": "Frozen ground truth for local test scoring"},
        "images": images,
    ...
    with redirect_stdout(buffer):
        ...
        evaluator.params.iouThrs = np.linspace(0.75, 0.95, 5)
        evaluator.evaluate()
...
def main() -> None:
    ...
    if gt.empty:
        raise ValueError("No ground truth rows remain after filtering.")
    ...
    score = coco_eval(pred_scored, gt, cat_to_name)

    summary = {
        "metric": "mAP@0.75:0.95",
        "score": score,
        "submission": display_path(inputs.submission),
        "ground_truth": display_path(inputs.ground_truth),
        "unknown_ignore_boxes": display_path(inputs.unknown_ignore_boxes),
        "submission_sha256": sha256_file(inputs.submission),
        "ground_truth_sha256": sha256_file(inputs.ground_truth),
        "out_dir": display_path(inputs.out_dir),
    ...
    (inputs.out_dir / "score.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    score_line = f"mAP@0.75:0.95 = {score:.6f}"
    (inputs.out_dir / "score.txt").write_text(score_line + "\n", encoding="utf-8")
    print(score_line)
