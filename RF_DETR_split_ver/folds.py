# rf-detr/folds.py
"""
fold 분할 관리: masked 접두어 인지 그룹화, 고정 분할(json) 저장/로드, 그룹 누수 점검, 분포 요약.

dataset.py의 make_folds()는 StratifiedGroupKFold로 분할을 "계산"하는 반면, 이 모듈은
그 분할 결과를 다루는 후속 단계(masked pool 인지 그룹화, 계정/세션/플랫폼 간 고정 공유,
검증)를 담당합니다. 배경:

1. dataset.make_folds()는 그룹 키를 "파일명의 '_0_2' 앞부분"(K코드 구성)으로 계산하는데,
   masked pool은 'msk_' 접두어 때문에 같은 구성의 원본 train 이미지와 "다른 그룹"으로
   계산되어 같은 구성(위도/촬영조건만 다른 이미지)이 train/valid에 갈라지는 누수가 생깁니다.
   -> make_folds_masked()는 그룹 키에서 접두어만 벗겨 계산합니다 (나머지 로직은 make_folds와 동일).
2. StratifiedGroupKFold가 "동일 입력 + 동일 seed"에서 같은 분할을 내놓는 것은 sklearn/numpy
   버전이 같을 때만 보장됩니다. 여러 계정/세션/플랫폼(Colab-Kaggle)에서 fold-matched
   앙상블을 하려면 분할 결과 자체를 json으로 고정해 공유해야 합니다.
...
def make_folds_masked(file_names, boxes_by_image, cats_by_image, cat2label,
    ...
    """dataset.make_folds()와 동일하되, 그룹 키에서 masked 접두어를 제거해 원본과 같은 구성 그룹으로 묶습니다.
...
def load_fold_split(path, file_names, n_splits):
    ...
    Returns:
        list: [(train_idx, val_idx), ...] (np.ndarray 인덱스, make_folds_masked와 동일 형태)
    """
