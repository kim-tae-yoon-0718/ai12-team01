# rf-detr/ensemble.py
"""
추론 결과 -> 제출 파일: WBF 융합, YOLO 앙상블 예측 수집, 제출 CSV 생성.

합집합 그대로 제출하면 같은 알약이 모델 수만큼 중복되어 mAP에서 FP로 깎입니다.
visualize.py의 _cluster_same_class_boxes(대표 박스 1개 선택)와 달리, WBF는 겹치는
박스들의 좌표를 confidence 가중 평균해 더 정교한 박스를 만듭니다 (외부 패키지
ensemble-boxes 사용). conf_type='avg'(기본값) 기준, 일부 모델만 잡은 박스는 score가
(동의 모델 수/전체 모델 수) 비율로 낮아져 "몇 개 모델이 동의했는지" 신호가 score에
자연스럽게 반영됩니다.
"""
import os
import re
...
def filter_valid_labels(pred_data, valid_labels):
    ...
    train.report_fold_result()가 valid 평가 때 하는 것과 동일한 정제 과정입니다.

    Args:
        pred_data: visualize.collect_predictions_ensemble() 계열의 반환값
...
def collect_predictions_ensemble_yolo(checkpoints, image_dir, conf_thr=0.05,
    ...
    visualize.collect_predictions_ensemble()과 동일한 반환 스키마입니다 (rfdetr의
    supervision.Detections 대신 ultralytics Results.boxes를 읽는 부분만 다름).
    ⚠ YOLO의 cls는 0-indexed라서, cat2label/label2cat 라벨 체계(1~N)에 맞추기 위해
      +1 해서 저장합니다.
