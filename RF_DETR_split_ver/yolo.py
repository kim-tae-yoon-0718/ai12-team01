# rf-detr/yolo.py
"""
YOLO(v8) 5-fold 학습·COCO->YOLO 포맷 변환.

train.py(train_fold/run_kfold)/model.py(get_rfdetr_model)의 패턴을 Ultralytics YOLO용으로
재정의한 것입니다 (라이브러리가 달라 원본 함수를 그대로 쓸 수 없어 별도 정의).
...
def build_yolo_fold(fold_idx, coco_dataset_dir, yolo_dataset_dir, label2cat):
    ...
    - dataset.build_coco()가 넣는 더미 배경 카테고리(id=0, name='pill')에는 박스가 하나도
    ...
    Args:
        ...
        coco_dataset_dir (str): dataset.write_fold_dirs()의 output_dir (COCO 포맷 fold 루트)
...
def run_folds_yolo(config, fold_yaml_paths, fold_indices=None, max_folds=None):
    """지정한 fold들을 학습+리포팅합니다 (train.run_kfold의 YOLO 버전).
...
def summarize_kfold_results_yolo(fold_metrics, tag):
    """fold별 mAP를 받아 평균±표준편차를 출력합니다 (utils.summarize_kfold_results의 YOLO 버전).
...
def summarize_per_class_yolo(fold_metrics, label2cat, label_counts, valid_pivot):
    """클래스(라벨)별 mAP@0.5:0.95를 fold 평균으로 집계합니다 (utils.summarize_per_class의 YOLO 버전).
    ...
    Args:
        ...
        label_counts (dict): 라벨별 전체 박스 수 (dataset.compute_label_counts 결과)
