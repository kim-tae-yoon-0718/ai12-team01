...
def show_gt_class_crops(boxes_by_image, cats_by_image, ids_by_image, img_path_index,
    ...
    함께 점검할 수 있습니다. crop 제목: 1줄째 = 파일명(전체), 2줄째 = annotation_id
    (train 박스는 원본 annotation JSON의 id, corrections의 add_boxes로 추가된 박스나
    ids_by_image에 없는 박스는 None).
    ...
    Args:
        ...
        ids_by_image (dict): file_name -> [annotation_id, ...] (boxes와 순서 동기화 상태).
            추적하지 않는 파이프라인이면 {}를 넘기면 됩니다 (제목에 ann_id 대신 None 표시).
...
def show_pred_class_crops(fused_data, label2cat, score_thr=0.3, max_per_class=None,
                          ncols=6, pad=8, classes=None):
    """예측(단일 모델 또는 WBF 융합 결과)을 클래스별 crop grid로 표시합니다.

    각 crop 제목: 1줄째 = 파일명, 2줄째 = confidence score. 제출 기준과 동일한
    score_thr로 점검하는 것을 권장합니다.

    Args:
        fused_data: fuse_predictions_wbf() 등의 반환값 (또는 같은 스키마의 단일 모델 예측)
        label2cat (dict): 모델 라벨 -> 원본 category_id
        score_thr (float): 표시할 예측의 최소 confidence
        max_per_class (int): 클래스당 표시할 crop 수 상한 (None이면 전부 표시, ncols 그리드)
