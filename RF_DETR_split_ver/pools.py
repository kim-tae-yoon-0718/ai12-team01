...
- 합성 pool(task1_synthesized/task2_synthesized): categories가 "라벨 네임스페이스"
  (id 1~N, name=원본 category_id). name 필드로 원본 id 공간으로 되돌린 뒤 병합해야
  dataset.py 파이프라인(build_coco 등)과 호환됩니다.
- masked pool(dataset-74-masked): categories의 id가 "원본 category_id 그대로" -> name
  매핑 불필요. 단, 파일명이 원본 train과 동일 체계(K코드+촬영조건)라 충돌할 수 있어
  전체 파일을 접두어('msk_')로 리네임한 사본을 스테이징 폴더에 만들어 병합합니다.
...
def load_pool_annotations(pool_ann_path):
    ...
    Returns:
        (boxes, cats, ids, meta, coco):
            ...
            coco (dict): 원본 COCO dict (categories가 라벨 매핑의 신뢰 소스)
    """
...
def merge_pool(boxes_by_image, cats_by_image, ids_by_image, img_meta,
    ...
    2. 박스 0개 이미지는 제외 - 학습에 기여하지 못하고 make_folds/make_folds_masked의
       층화 기준(이미지 내 최희소 클래스) 계산에서 에러가 나기 때문
...
def merge_masked_pool(boxes_by_image, cats_by_image, ids_by_image, img_meta,
    ...
    안전장치:
    1. masked pool의 category_id가 허용 집합(라벨 매핑의 원본 id 공간)에 전부 포함되는지 assert
       -> 라벨 공간 json을 잘못 넣은 경우 등 포맷 착오를 학습 전에 잡습니다.
    ...
    Args:
        ...
        stage_dir (str): 리네임 사본을 만들 스테이징 폴더 (dataset.cache_images에 그대로 전달 가능)
        cats_allowed (set): 허용 category_id 집합 (예: 합성 pool_coco categories의 name 집합)
...
def apply_exclusions(exclude_files, boxes_by_image, cats_by_image, ids_by_image, img_meta):
    ...
    ⚠ 반드시 라벨 매핑/fold 분할 "이전"에 실행해야 하며, 고정 fold 분할(json)을 쓰는
      파이프라인에서는 json export 시점과 동일한 목록이어야 분할-데이터 일치 검증을 통과합니다.
...
def build_cat2label_74(pool_coco, train_cats, cats_by_image=None):
    ...
    Returns:
        (cat2label, label2cat, all_cats): 원본 id<->라벨(1~74) 매핑과
            dataset.build_coco()에 넘길 카테고리 목록(라벨 오름차순)
    """
